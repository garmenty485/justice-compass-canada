"""
Create or update the justice-compass-prod-pipeline Job (01 → 02 → 03 → 05 → 09).

No schedule — GitHub Actions triggers run-now after seed + Repos pull.
"""
...
JOB_NAME = "justice-compass-prod-pipeline"
...
NOTEBOOKS = (
    ...
    ("deploy_serving", "05_deploy_serving"),
    ("sync_cases", "09_sync_cases_prod"),
)
...
def notebook_paths(repo_root: str) -> dict[str, str]:
    base = f"{repo_root}/databricks/prod_notebooks_job"
...
def build_job_settings(repo_root: str, *, name: str = JOB_NAME) -> dict[str, Any]:
    ...
    return {
        "name": name,
        "description": (
            "Prod pipeline: Bronze → Silver → Gold → Serving deploy → Synced Table"
        ),
        ...
        "tasks": [
            {
                ...
                "notebook_task": {
                    "notebook_path": paths["bronze_ingest"],
                    "source": "WORKSPACE",
                },
            },
            {
                ...
                "notebook_task": {
                    "notebook_path": paths["silver_transform"],
                    "source": "WORKSPACE",
                },
            },
            {
                ...
                "notebook_task": {
                    "notebook_path": paths["gold_embed"],
                    "source": "WORKSPACE",
                },
            },
            {
                "task_key": "deploy_serving",
                "depends_on": [{"task_key": "gold_embed"}],
                "notebook_task": {
                    "notebook_path": paths["deploy_serving"],
                    "source": "WORKSPACE",
                },
            },
            {
                "task_key": "sync_cases",
                "depends_on": [{"task_key": "deploy_serving"}],
                "notebook_task": {
                    "notebook_path": paths["sync_cases"],
                    "source": "WORKSPACE",
                    "base_parameters": {
                        "SYNCED_TABLE_UC_NAME": DEFAULT_SYNCED_TABLE_UC_NAME,
                    },
                },
            },
        ],
