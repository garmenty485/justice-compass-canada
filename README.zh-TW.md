...
2. Branch：`main`；**Build output directory**：`cloudflare/pages`
3. Deploy
...
詳細疑難排解：[`docs/DEPLOY_PHASE2.md`](docs/DEPLOY_PHASE2.md)
...
| Secret | 用途 |
|--------|------|
| `CLOUDFLARE_API_TOKEN` | push 到 `main` 自動部署 Worker/Pages |
| `GEMINI_API_KEY` | PR 自動 AI review |

## 步驟 8 — 完整自動化：排程 prod pipeline（進階，選填）

跟作者本人一樣的自動化：**每 2 小時**自動 seed 測試案例 → push → 通知 Databricks pull → 觸發 Job 跑完整 pipeline（`01→02→03→05→09`）。

**預設是關閉的**（`.github/workflows/prod-seed-and-pipeline.yml` 的 `schedule` 已註解掉），因為需要先完成步驟 4–6。要達到跟作者一樣的最終狀態：
