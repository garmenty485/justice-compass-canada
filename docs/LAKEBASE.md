...
> 本專案採用 Lakebase 做 Lake↔Base 整合；不做 AI Search（現有 Delta cosine + Model Serving 已足夠支撐 RAG 檢索）。
...
本專案使用 **自訂 Postgres ROLE + native PASSWORD**（固定密碼），**不用** OAuth / Service Principal token rotation：
...
**摘要**：判例 metadata 透過 **Synced Tables** 從 Delta 進 Lakebase，供前端低延遲讀取；每次 RAG 查詢寫入 Lakebase `query_logs` 做稽核。反向的 **Lakebase CDF**（Base 回流 Lake）已設定並驗證過設定步驟本身無誤，但 Free Edition 的 catalog 只有 default storage，CDF 目的地需要自訂雲端儲存位置，架構上不支援（詳見下方「CDF 驗收狀態」）。
...
| Delta 稽核表 `lb_query_logs_history` | ❌ Free Edition 架構限制，非 bug |
| Audit 展示 | 直接在 Lakebase SQL Editor 查 `query_logs`（見下方 workaround） |
...
**若切換到付費 workspace（catalog 掛自訂雲端儲存位置）**：Pull → 跑 `10` → 確認 `justice_compass_lakebase.default.lb_query_logs_history` 有 `_pg_change_type` 列。
